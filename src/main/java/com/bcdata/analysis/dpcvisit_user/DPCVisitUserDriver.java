...
public class DPCVisitUserDriver {

    private static final Logger logger = Logger.getLogger (DPCVisitUserDriver.class);
...
//            conf.addResource (IOUtils.toInputStream (FileUtils.readFileToString (new File ("url_tags.txt"), "utf8")));
            ...
            Job job = Job.getInstance (jobConf, "visit user");
            job.setJarByClass (DPCVisitUserDriver.class);
            job.setMapperClass (DPCVisitUserMapper.class);
            ...
            job.setReducerClass (DPCVisitUserReducer.class);
